...
/**
 * Адаптер для интеграции с ВКонтакте (VK)
 * Обрабатывает интенты от VK для получения аудио данных
 */
class VKAdapter : MessengerAdapter, ContextAware {
    
    companion object {
        private const val TAG = "VKAdapter"
        private const val VK_PACKAGE_NAME = "com.vkontakte.android"
        
        // Константы для ВКонтакте
        const val VK_ACTION_SEND = "android.intent.action.SEND"
        const val VK_EXTRA_STREAM = "android.intent.extra.STREAM"
        
        // MIME типы для ВКонтакте
    ...
    override fun getAudioStream(intent: Intent): InputStream? {
        try {
            ...
            uri = intent.getParcelableExtra(Intent.EXTRA_STREAM) ?:
                  intent.getParcelableExtra(VK_EXTRA_STREAM)
            ...
            android.util.Log.e(TAG, "Error getting audio stream from VK", e)
            return null
        }
    }
    ...
    override fun supportsIntent(intent: Intent): Boolean {
        try {
            ...
            if (!packageName.isNullOrEmpty()) {
                when {
                    packageName.contains("vk", ignoreCase = true) -> return true
                }
            }
            ...
            android.util.Log.e(TAG, "Error checking VK intent support", e)
            return false
        }
    }
    ...
    override fun isAvailable(): Boolean {
        return try {
            ...
            packageManager.getPackageInfo(VK_PACKAGE_NAME, 0)
            true
        } catch (e: PackageManager.NameNotFoundException) {
            false
        } catch (e: Exception) {
            android.util.Log.e(TAG, "Error checking VK availability", e)
            false
        }
    }
